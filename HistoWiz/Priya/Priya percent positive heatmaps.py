...
Originally created for Sandra, goal is to create heatmaps of percent positive scoring, where the columns are
the different code categories, and the rows are the percent positive combinations. Initially, the percent
positive combinations will be limited to single positive, but we'd like to include double/triple positives.
Additionally, we'd like to include choice of denominator (e.g. num detection vs area column

The input for this will be the processed annotation measurements. We will reuse some code for the
percent single positive boxplots. 
"""
#%% Import libraries
import pandas
import pandas as pd
...
figpath=r'C:\Users\Mark Zaidi\Documents\QuPath\HistoWiz\Priya_v2\figures\percent positive heatmaps\Code'
groupby='Code'
...
norm_heatmap[grouped_df.columns[1:]]=norm_heatmap[grouped_df.columns[1:]].apply(lambda x: (x-x.mean())/x.std(), axis = 0)
...
h=sns.heatmap(data=norm_heatmap[heatmap_cols].transpose(),annot=grouped_df[heatmap_cols].transpose(),square=True,cmap="viridis",cbar=False,annot_kws={"size": 6},fmt='.2g')
